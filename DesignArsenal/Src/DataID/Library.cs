...
namespace DesignArsenal.DataID
{
	...
	public class Library
	{
		...
		{
			string json = File.ReadAllText(Consts.AppDir_Shared + "id_flaticons.json");
			...
			foreach(var source in _lib.sources)
			{
				...
				{
					icon.hash = Utils.CalculateMD5Hash(string.Join("", icon.arr_svgpath) + $"-{icon.arr_tags[0]}-{source.name})");
